...
def build_mixed_dataset(args, preprocess, target_transform, logger):
    source_data = Dataset(
    ...
    summary = [(args.dataset, args.train_data_path, args.primary_mode, len(source_data), len(source_data))]

    if args.mix_data_paths:
        mix_paths = args.mix_data_paths
        mix_names = args.mix_dataset_names
        mix_modes = args.mix_modes or ["test"] * len(mix_paths)

        if not (len(mix_paths) == len(mix_names) == len(mix_modes)):
            raise ValueError("mix_data_paths, mix_dataset_names, and mix_modes must have the same length")

        rng = random.Random(args.seed)
        for mix_path, mix_name, mix_mode in zip(mix_paths, mix_names, mix_modes):
            mix_dataset = Dataset(
                root=mix_path,
                transform=preprocess,
                target_transform=target_transform,
                dataset_name=mix_name,
                mode=mix_mode,
            )
            subset_len = max(1, int(len(mix_dataset) * args.mix_ratio))
            indices = list(range(len(mix_dataset)))
            rng.shuffle(indices)
            sampled_dataset = Subset(mix_dataset, indices[:subset_len])
            datasets.append(sampled_dataset)
            summary.append((mix_name, mix_path, mix_mode, len(mix_dataset), subset_len))

    logger.info("Training data composition:")
    for name, path, mode, total_len, used_len in summary:
        logger.info("  dataset=%s mode=%s total=%d used=%d path=%s", name, mode, total_len, used_len, path)
...
def train(args):
    ...
    train_data = build_mixed_dataset(args, preprocess, target_transform, logger)
    ...
    optimizer = torch.optim.Adam(
        list(prompt_learner.parameters()) + list(visual_prompt.parameters()) + list(adaptive_fusion.parameters()),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser("AnomalyCLIP-VisualPrompt-GatedFusion-Mix", add_help=True)
    parser.add_argument("--train_data_path", type=str, default="./data/mvtec", help="primary train dataset path")
    parser.add_argument("--save_path", type=str, default="./checkpoint/noForzen/hsf_visual_prompt_gate_mix", help="path to save results")
    parser.add_argument("--dataset", type=str, default="mvtec", help="primary train dataset name")
    parser.add_argument("--primary_mode", type=str, default="train", help="split name for primary train dataset")
    parser.add_argument("--mix_data_paths", type=str, nargs="*", default=[], help="additional dataset paths to mix into training")
    parser.add_argument("--mix_dataset_names", type=str, nargs="*", default=[], help="dataset names for the mixed datasets")
    parser.add_argument("--mix_modes", type=str, nargs="*", default=[], help="split names for the mixed datasets")
    parser.add_argument("--mix_ratio", type=float, default=0.1, help="ratio sampled from each mixed dataset")
