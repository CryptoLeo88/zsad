...
def train(args):
    ...
    optimizer = torch.optim.Adam(
        list(prompt_learner.parameters()) + list(visual_prompt.parameters()),
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        for items in tqdm(train_dataloader):
            ...
            enhanced_image_features, visual_prompt_tokens, _ = visual_prompt(image_features, text_features)

            text_probs = enhanced_image_features.unsqueeze(1) @ text_features.permute(0, 2, 1)
            ...
            cur_image_feature = alpha * clustered_feature + (1 - alpha) * enhanced_image_features
            ...
            if 'llm_embedding' in items:
                loss_kd_value = loss_kd(items['llm_embedding'].to(device), prompts_pos, prompts_neg)
                loss += loss_kd_value

            # Keep the visual prompt close to the text-guided prompt while preserving the original image feature.
            vp_target = visual_prompt_tokens.detach() / visual_prompt_tokens.detach().norm(dim=-1, keepdim=True)
            vp_loss = F.mse_loss(enhanced_image_features, vp_target)
            vp_loss_list.append(vp_loss.item())
            total_loss = loss + image_loss + args.visual_prompt_loss_weight * vp_loss
        ...
        if (epoch + 1) % args.print_freq == 0:
            logger.info(
                'epoch [{}/{}], loss:{:.4f}, image_loss:{:.4f}, vp_loss:{:.4f}'.format(
                    epoch + 1,
                    args.epoch,
                    np.mean(loss_list),
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser("AnomalyCLIP-VisualPrompt", add_help=True)
    parser.add_argument("--train_data_path", type=str, default="./data/mvtec", help="train dataset path")
    parser.add_argument("--save_path", type=str, default='./checkpoint/noForzen/hsf_visual_prompt', help='path to save results')
