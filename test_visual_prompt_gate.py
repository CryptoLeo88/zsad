...
def test(args):
    ...
    adaptive_fusion.load_state_dict(checkpoint["adaptive_fusion"])
    ...
    for items in tqdm(test_dataloader):
        ...
        with torch.no_grad():
            ...
            aligned_visual_features, _, _ = visual_prompt(image_features, text_features)
            gated_visual_features, _ = adaptive_fusion.fuse_visual_prompt(image_features, aligned_visual_features)
            ...
            final_image_feature, _ = adaptive_fusion.fuse_cluster_feature(gated_visual_features, clustered_feature)

            text_probs = final_image_feature.unsqueeze(1) @ text_features.permute(0, 2, 1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("AnomalyCLIP-VisualPrompt-Gate-Test", add_help=True)
