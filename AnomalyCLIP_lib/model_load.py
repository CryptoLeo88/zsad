...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu",
         design_details=None, jit: bool = False, download_root: str = None):
    """Load a CLIP model
    ...
    if name in _MODELS:
        ...
        model_path = _download(_MODELS[name],
                               download_root or os.path.expanduser("/remote-home/iot_zhouqihang/root/.cache/clip"))
...
def compute_similarity(image_features, text_features, t=2):
    t = text_features.t().unsqueeze(0)
    prob_1 = 100.0 * image_features @ t
